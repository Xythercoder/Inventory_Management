    ...
    useEffect( () => {
        const fetchData = async () => {
            try {
                ...
                const counter = await axios.get( apiEndPoint.inventories_approved_count, config() );
                ...
                if ( res.data && counter.data ) {
                    ...
                    const labels = res.data.map( ( item ) => {
                        const dateStr = item.updated_at; // Assuming 'updated_at' is the correct date field
                    ...
                    const counts = Array.isArray( counter.data )
                        ? counter.data.map( ( item ) => item.approved_count ) // If it's an array
                        : [ counter.data.approved_count ]; // If it's an object, get the count directly
